                        ...
                        <ol class="breadcrumb mb-4"></ol>
                            ...
                            <i class="fas fa-table me-1"></i>
                                ...
                                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                        ...
                                        <a href="{{ url('cetak')}}" target="_blank" class = "btn btn-secondary">Laporan</a>
                                        @endif
                                    </div>  
                                ...
                                <table class="table table-bordered " id="datatablesSimple">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Stok</th>
                                            <th>Satuan</th>
                                            @if (auth()->user()->level=="Admin" and "Staff")
                                            <th> </th>
                                            @endif
                                        </tr>
                                    </thead>
                                    ...
                                    @if($data_buku -> count() > 0)
                                            ...
                                            @foreach($data_buku as $rs)
                                                <tr>
                                                    ...
                                                    @if($buku_keluar->count() > 0)
                                                        @php
                                                            $totalJumlahKeluar = $buku_keluar->where('kode_buku', $rs->kode_buku)->sum('jumlah_keluar');
                                                        @endphp

                                                        {{ $rs->jumlah_masuk - $totalJumlahKeluar }}
                                                    @else
                                                        {{ $rs->jumlah_masuk }}
                                                    @endif
                                                    ...
                                                    <td class="align-middle">
                                                        <div class="btn-group" role="group" aria-label="Basic example">
                                                            <a href="{{ route('databuku.edit', $rs->kode_buku)}}" type="button" class="btn btn-primary">Edit</a>
                                                            <form action="{{ route('databuku.destroy', $rs->kode_buku) }}" method="POST" type="button" class="btn btn-danger p-0" onsubmit="return confirm('Delete?')">
                                                                @csrf
                                                                @method('DELETE')
        ...
        <!-- <div class="modal fade" id="reg-modal">
            <div class="modal-dialog">
                ...
                <div class="modal-footer">
                    ...
                    <a href="{{url('stok')}}" type="button" class="btn btn-danger">Hapus</a>
                </div>
                </div>
            </div>
        </div> -->
